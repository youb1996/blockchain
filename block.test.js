...
describe('Block', () => {
    ...
    describe('mineBlock()', () => {
        ...
        it('adjusts the difficulty',()=>{
            const possibleResulte = [lastBlock.difficulty+1, lastBlock.difficulty-1];
            expect(possibleResulte.includes(minedBlock.difficulty)).toBe(true);
        });

    });

    describe('adjustDifficulty()', () => {
        it('raises the difficulty for a quickly mained block', () => {
            expect(Block.adjustDifficulty({ originalBlock: block, timestamp: block.timestamp + MINE_RATE - 100 })).toEqual(block.difficulty + 1)
        });

        it('lowers the difficulty for a slowly mained block', () => {
            expect(Block.adjustDifficulty({ originalBlock: block, timestamp: block.timestamp + MINE_RATE + 100 })).toEqual(block.difficulty - 1)
        })
